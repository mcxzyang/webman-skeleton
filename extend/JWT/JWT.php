...
class JWT
{
     ...
     */
    public const REFRESH = 2, ACCESS = 1;
    ...
    public function make(array $extend, int $access_exp = 0, int $refresh_exp = 0)
    {
        ...
        if ($this->redis) {
            ...
            //存储session
            session()->set("{$this->prefix}_token_{$this->guard}", $accessToken);
        } else {
            //存储session
            session()->set("{$this->prefix}_token_{$this->guard}", $accessToken);
        }
    ...
    protected function getTokenFormHeader()
    {
        ...
        $token = $token ?? session("{$this->prefix}_token_{$this->guard}", null);
        if (empty($token)) {
            $token = null;
    ...
    public function logout($all = false)
    {
        ...
        if (isset($this->config['redis']) && $this->config['redis']) {
            ...
            if ($all) {
                Redis::hDel("{$this->prefix}_token_{$this->guard}", $id);
            } else {
                $list = Redis::hGet("{$this->prefix}_token_{$this->guard}", $id);
                if ($list) {
                    ...
                    if (count($tokenList) == 0) {
                        ...
                        Redis::hSet("{$this->prefix}_token_{$this->guard}", $id, serialize($tokenList));
                    }
                }
            }
        ...
        //清理session数据
        session()->forget("{$this->prefix}_token_{$this->guard}");
    }
        ...
        if ($list != null) {
            ...
            if (is_array($tokenList)) {
                if ($maxNum === -1) { //不限制
                    ...
                    Redis::hSet("{$this->prefix}_token_{$this->guard}", $id, serialize([$defaultList]));
